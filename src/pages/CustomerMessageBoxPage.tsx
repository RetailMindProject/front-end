...
import { Inbox as InboxIcon, MessageSquare, RefreshCcw, Send } from "lucide-react";
...
export default function CustomerMessageBoxPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const [inboxRes, sentRes] = await Promise.all([messagesApi.getInboxMessages(), messagesApi.getOutboxMessages()]);
  ...
  return (
    <div className="pt-2">
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <h2 className="text-xl sm:text-2xl font-bold text-slate-900">Message Box</h2>
        <button
          onClick={load}
          className="inline-flex items-center gap-2 px-3 py-2 rounded-lg bg-white/70 border border-slate-200 text-slate-700 hover:bg-white transition-all duration-200 shadow-sm text-sm font-semibold"
          aria-label="Refresh"
        >
