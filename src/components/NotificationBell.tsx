            ...
            {items.map((n) => (
              ...
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="font-semibold text-slate-800 group-hover:text-blue-700 transition-colors duration-200">{n.title}</span>
                  <span className="text-xs text-slate-400 group-hover:text-slate-600 transition-colors duration-200">{n.createdAt}</span>
                </div>
                <div className="text-slate-600 mb-1 group-hover:text-slate-700 transition-colors duration-200">{n.body}</div>
                <div className="text-xs text-slate-500 group-hover:text-slate-600 transition-colors duration-200">From: {n.from}</div>
              </li>
            ))}
          </ul>
